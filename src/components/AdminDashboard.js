...
const AdminDashboard = () => {
  ...
  const fetchAllDevices = async () => {
    try {
      // Fetch lights
      const lightsResponse = await fetch("http://localhost:8080/light/all", {
        headers: {
      ...
      // Fetch doors
      const doorsResponse = await fetch("http://localhost:8080/door/all", {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
      });
      ...
      // Fetch cameras
      const camerasResponse = await fetch("http://localhost:8080/camera/all", {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
      });
