...
function SchedulePopup({ isOpen, onClose, deviceType, deviceId, userId }) {
  ...
  const fetchSchedules = async () => {
    try {
      const response = await fetch(`${BACKEND_URL}/schedules/device/${deviceType.toLowerCase()}/${deviceId}`);
  ...
  const toggleSchedule = async (scheduleId, isActive) => {
    try {
      const response = await fetch(`${BACKEND_URL}/schedules/${scheduleId}/toggle?isActive=${!isActive}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        }
